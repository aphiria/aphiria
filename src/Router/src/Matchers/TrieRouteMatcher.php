...
final class TrieRouteMatcher implements IRouteMatcher
{
    ...
    public function matchRoute(string $httpMethod, string $host, string $path, array $headers = []): RouteMatchingResult
    {
        ...
        foreach (self::getMatchCandidates___________New($this->rootNode, $pathSegments, $pathSegmentsCount, 0, $hostSegments, $routeVars) as $candidate) {
     ...
     */
    private static function getMatchCandidates___________New(
        TrieNode $node,
        array $segments,
        int $segmentCount,
        int $segmentIter,
