...
use Aphiria\Routing\UriTemplates\Compilers\Tries\ParserTrieCompiler;
...
class ParserTrieCompilerTest extends TestCase
{
    private ParserTrieCompiler $compiler;
    ...
    protected function setUp(): void
    {
        ...
        $this->parser->method('parse')
            ->willReturn($this->ast);
        $this->compiler = new ParserTrieCompiler($this->constraintFactory, $this->parser, $this->lexer);
    ...
    public function testConstructingCompilerWithoutConstraintsStillRegistersDefaultOnes(): void
    {
        ...
        $this->lexer->expects($this->once())
            ...
            ->with($pathTemplate)
            ->willReturn(new TokenStream([]));
        $compiler = new ParserTrieCompiler(null, $this->parser, $this->lexer);
