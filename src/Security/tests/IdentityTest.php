...
class IdentityTest extends TestCase
{
    ...
    {
        $identity = new ClaimsIdentity([], $expectedAuthenticationSchemeName);
    ...
    {
        $identityWithoutNameIdentifierClaim = new ClaimsIdentity([]);
        $this->assertNull($identityWithoutNameIdentifierClaim->getName());
        $identityWithNameIdentifierClaim = new ClaimsIdentity([new Claim(ClaimType::NameIdentifier, '123', 'http://example.com')]);
    ...
    {
        $identityWithoutNameClaim = new ClaimsIdentity([]);
        $this->assertNull($identityWithoutNameClaim->getName());
        $identityWithNameClaim = new ClaimsIdentity([new Claim(ClaimType::Name, 'Dave', 'http://example.com')]);
    ...
    {
        $authenticatedIdentity = new ClaimsIdentity([], 'foo');
        $this->assertTrue($authenticatedIdentity->isAuthenticated());
        $unauthenticatedIdentity = new ClaimsIdentity([]);
